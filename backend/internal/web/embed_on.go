...
type PublicHomeSettingsProvider interface {
	GetPublicHomeSettingsForInjection(ctx context.Context) (any, error)
}
...
type FrontendServer struct {
	...
	homeSettings PublicHomeSettingsProvider
...
func NewFrontendServer(settingsProvider PublicSettingsProvider) (*FrontendServer, error) {
	...
	homeCache := NewHTMLCache()
	homeCache.SetBaseHTML(baseHTML)
	...
	if homeSettings, ok := settingsProvider.(PublicHomeSettingsProvider); ok {
		server.homeSettings = homeSettings
	}
	return server, nil
}
...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if cleanPath == "index.html" || !s.fileExists(cleanPath) {
			s.serveIndexHTML(c, isPublicHomeRoute(path))
			return
		}
...
func (s *FrontendServer) serveIndexHTML(c *gin.Context, publicHome bool) {
	// Get nonce from context (generated by SecurityHeaders middleware)
	nonce := middleware.GetNonceFromContext(c)
	cache := s.cache
	if publicHome && s.homeCache != nil {
		cache = s.homeCache
	}
	...
	settings, err := s.settingsForInjection(ctx, publicHome)
	if err != nil {
	...
	cache.Set(rendered, settingsJSON)
...
func (s *FrontendServer) settingsForInjection(ctx context.Context, publicHome bool) (any, error) {
	if publicHome && s.homeSettings != nil {
		return s.homeSettings.GetPublicHomeSettingsForInjection(ctx)
	}
	return s.settings.GetPublicSettingsForInjection(ctx)
}

func isPublicHomeRoute(path string) bool {
	trimmed := strings.TrimSpace(path)
	if trimmed == "" || trimmed == "/" || trimmed == "/home" {
		return true
	}
	return strings.TrimSuffix(trimmed, "/") == "/home"
}
...
func injectSiteTitle(html, settingsJSON []byte) []byte {
	...
	title := cfg.SiteName
	if strings.EqualFold(title, "Sub2API") {
		title = "Qinglan Knowledge Base"
	}
	newTitle := []byte("<title>" + title + "</title>")
